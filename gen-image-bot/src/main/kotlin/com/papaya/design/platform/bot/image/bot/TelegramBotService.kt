...
private val WELCOME_MESSAGE_TEXT =
...
Нажмите /start - создадим ваш интерьер мечты вместе.""".trimIndent()
...
""".trimIndent(),
    ...
    private fun handlePhotoMessage(chatId: Long, photos: List<PhotoSize>) {
        try {
            ...
            bot.sendMessage(
                chatId = ChatId.fromId(chatId),
                text = "Получил изображение! Начинаю генерацию реалистичного интерьера, займет примерно минутку..."
            )
