...
object PlannedRealisticInterior {
    object Text {
        val START_GENERATION = """
    - Загрузи мудбоард, который тебе хотелось бы воплотить в жизнь.
...
""".trimIndent()
    ...
    object Prompt {
        val SYSTEM_PROMPT = """
            Ты опытный дизайнер и 3D-визуализатор. Ты получишь в работу мудбоард с предметами которые необходимо добавить в интерьер. 
            На 3D-изображении должны быть в точности отражены предметы, которые уже есть на мудбоарде. 
            Ты не должен отклоняться от технического задания и добавлять предметы интерьера на свое усмотрение.
            Предметы с мудбоарда должны сохранить свой цвет, свою форму, фактуру и прочие характеристики.
            Вторым изображением ты получишь план помещения.
            Исходя из плана, ты должен раставить предметы с мудбоарда и дать фото интерьера.
